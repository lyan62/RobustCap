...
class ThisOPTDecoderLayer(OPTDecoderLayer):
    def __init__(self, config):
        ...
        if config.add_cross_attention:
            ...
            self.encoder_attn_layer_norm = nn.LayerNorm(self.embed_dim, elementwise_affine=True)#config.layer_norm_elementwise_affine)
...
class XASA_OPTDecoderLayer(OPTDecoderLayer):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        cross_attn_present_key_value = None
        cross_attn_weights = None
        if encoder_hidden_states is not None: # encoder_hidden_states: B x 50 x 768
            residual = hidden_states # B x L x H (1024)
            hidden_states = self.encoder_attn_layer_norm(hidden_states)
